...
import {
  ...
  getActivePaymentPromisesForInvoices,
...
const PROCESSING_STALE_THRESHOLD_MS =
  PROCESSING_STALE_THRESHOLD_MINUTES * 60 * 1000;
...
async function makeReminderSlotId(
  ownerId: string,
  invoiceId: string,
  localDate: string,
): Promise<string> {
...
export async function runReminderEngineForOwner(
  ...
  const invoices =
    await deps.listOverdueInvoices(
      ownerId,
      localDate,
    );
  ...
  const paymentSettings =
    await deps.getBusinessPaymentSettings(
      ownerId,
    );
  ...
  for (const invoice of invoices) {
    if (
      invoice.owner_id !== ownerId ||
      !invoiceCollectible(invoice) ||
  ...
  for (const candidate of candidates) {
    ...
    const activePromise =
      activePaymentPromises.get(
        invoice.id,
      ) ?? null;

    let paymentPromiseContext:
      | {
          promiseDate: string;
          status:
            | "due_today"
    ...
    if (activePromise) {
      ...
      if (
        ...
        await deps.fulfillPaymentPromise(
          ownerId,
          invoice.id,
          scheduledAt,
        );
    ...
    const claim =
      await deps.claimReminderAttempt({
        slot_id: slotId,
        owner_id: ownerId,
...
export async function processReminderEngineForOwner(
  args: {
  ...
  const deps: ReminderEngineDependencies =
    {
      async getReminderSettings(
        ...
        if (error) {
          throw new Error(
            `Failed to load reminder settings for owner ${ownerId}: ${error.message}`,
          );
        }
      ...
      async listOverdueInvoices(
        ...
        if (error) {
          throw new Error(
            `Failed to load overdue invoices for owner ${ownerId}: ${error.message}`,
          );
        }
      ...
      async getBusinessPaymentSettings(
        ...
        if (error) {
          throw new Error(
            `Failed to load business payment settings for owner ${ownerId}: ${error.message}`,
          );
        }
      ...
      async getActivePaymentPromises(
        ownerId,
        invoiceIds,
      ) {
        return getActivePaymentPromisesForInvoices(
          {
            supabase:
              args.supabase,
            ownerId,
      ...
      async claimReminderAttempt(
        ...
        if (
          ...
          throw new Error(
            `Failed to claim reminder slot for invoice ${row.invoice_id}: ${insertError.message}`,
          );
        }
        ...
        if (reclaimError) {
          throw new Error(
            `Failed to reclaim reminder slot for invoice ${row.invoice_id}: ${reclaimError.message}`,
          );
        }
        ...
        if (staleReclaimError) {
          throw new Error(
            `Failed to recover stale reminder slot for invoice ${row.invoice_id}: ${staleReclaimError.message}`,
          );
        }
        ...
        if (existingError) {
          throw new Error(
            `Failed to load existing reminder slot for invoice ${row.invoice_id}: ${existingError.message}`,
          );
        }
      ...
      async finalizeReminderAttempt(
        ...
        if (error) {
          throw new Error(
            `Failed to finalize reminder slot ${input.slotId}: ${error.message}`,
          );
        }

        if (!data?.id) {
          throw new Error(
            `Reminder slot ${input.slotId} was not in processing state during finalize.`,
          );
        }
      },
  ...
  return runReminderEngineForOwner(
    args.ownerId,
    deps,
    args.now ?? new Date(),
  );
